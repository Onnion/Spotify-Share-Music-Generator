...
router.post("/", async function (req, res) {
  try {
    ...
    const { preview_url, album, name } = await spotifyService.getTrack(
      track_url
    );
    ...
    res
      ...
      .send({
        error: false,
        status: "published",
        data: { album_name: album.name, name, preview_url, thumb },
      });
  } catch (error) {
    res
      .status(500)
      .send({ error: true, status: "unpublished", message: error.message });
  }
});
